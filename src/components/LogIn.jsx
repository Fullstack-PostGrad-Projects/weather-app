...
const Login = ({ setToken, token }) => {
  ...
  const handleSignUp = async (event) => {
    event.preventDefault();
    console.log(event.target, 'hereeeeee')
    const password = event.currentTarget.floatingInputPassword.value;
    const username = event.currentTarget.floatingInputEmail.value;
  ...
  const handleChangePass = async (event) =>{
    ...
    setPassword(event.currentTarget.floatingInputPassword.value)
  }
  ...
  return (
    <Container>
      <Form onSubmit={handleSubmit} id="formLayout">
        ...
        <Form.Group className="mb-3" controlId="formBasicPassword">
          <FloatingLabel
            ...
            className="mb-3"
          >
            <Form.Control onChange={handleChangePass} type="password" placeholder="Password" required />
          </FloatingLabel>
        </Form.Group>
      ...
      <div className="d-grid gap-2">
        <Button onClick={handleSignUp} variant="outline-primary" size="lg">
          Sign up
        </Button>
      </div>
    </Container>
